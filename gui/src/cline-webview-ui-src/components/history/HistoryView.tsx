...
import { formatLargeNumber, formatSize } from "@webview-ui/utils/format";
...
const HistoryView = ({ onDone }: HistoryViewProps) => {
  ...
  const [pendingFavoriteToggles, setPendingFavoriteToggles] = useState<
  ...
  const handleShowTaskWithId = useCallback((id: string) => {
    TaskServiceClient.showTaskWithId({ value: id }).catch((error) =>
        ...
        <div style={{ padding: "5px 17px 6px 17px" }}>
          ...
          >
            ...
            <VSCodeRadioGroup
              ...
              onChange={(e) =>
                setSortOption(
                  (e.target as HTMLInputElement).value as SortOption,
                )
              }
            >
...
const ExportButton = ({ itemId }: { itemId: string }) => (
  <VSCodeButton
    ...
    onClick={(e) => {
      e.stopPropagation();
      TaskServiceClient.exportTaskWithId({ value: itemId }).catch((err) =>
